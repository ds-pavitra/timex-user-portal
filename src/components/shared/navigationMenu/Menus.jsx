        ...
        <>
            {menuList.map(({ dropdownMenu, id, name, label, path, icon }) => {
                if (!dropdownMenu || dropdownMenu.length === 0) {
                    return (
                        <li key={id} className={`nxl-item ${activeParent === name ? "active" : ""}`}>
                            <Link to={path} className="nxl-link">
                                <span className="nxl-micon">{getIcon(icon)}</span>
                                <span className="nxl-mtext" style={{ paddingLeft: "2.5px" }}>{label || name}</span>
                            </Link>
                        </li>
                    );
                }
                                ...
                                return (
                                    <Fragment key={id}>
                                        {subdropdownMenu.length ? (
                                            ...
                                            <li className={`nxl-item ${pathName === path ? "active" : ""}`}>
                                                <Link className="nxl-link" to={path}>{name}</Link>
                                            </li>
                                        )}
                                    </Fragment>
                                );
